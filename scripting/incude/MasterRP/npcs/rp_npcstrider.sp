...
public void initNpcStrider()
{
	...
	//Entity Event Hook:
	HookEntityOutput("npc_strider", "OnDeath", OnStriderDied);
}
...
//Event Damage:
public Action OnClientDamageStrider(int Ent, int &Client, int &inflictor, float &damage, int &damageType)
{
...
public void OnStriderDied(const char[] Output, int Caller, int Activator, float Delay)
{
	...
	if(IsValidEdict(Activator))
	{
		...
		//Print:
		CPrintToChatAll("\x07FF4040|RP|\x07FFFFFF |\x07FF4040ATTENTION\x07FFFFFF| - %N Has took out the Strider!", Activator);
	}
...
public int CreateNpcStrider(const char[]Model, float Position[3], float Angles[3], int Health)
{
	...
	if(NPC > 0)
	{
		...
		DispatchKeyValue(NPC, "name", "npc_strider");
