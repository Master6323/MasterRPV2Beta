...
public void initNpcHeadCrabBlack()
{
	...
	//Entity Event Hook:
	HookEntityOutput("npc_HeadCrab_black", "OnDeath", OnHeadCrabBlackDied);
}
...
//Event Damage:
public Action OnClientDamageHeadCrabBlack(int Client, int &attacker, int &inflictor, float &damage, int &damageType)
{
...
public void OnHeadCrabBlackDied(const char[] Output, int Caller, int Activator, float Delay)
{
	...
	if(IsValidEdict(Activator))
	{
		...
		//Print:
		CPrintToChatAll("\x07FF4040|RP|\x07FFFFFF |\x07FF4040ATTENTION\x07FFFFFF| - %N Has took out the HeadCrabBlack!", Activator);
	}
...
public int CreateNpcHeadCrabBlack(const char[] Model, float Position[3], float Angles[3], int Health)
{
	...
	if(NPC > 0)
	{
		...
		DispatchKeyValue(NPC, "name", "npc_headcrabblack");
