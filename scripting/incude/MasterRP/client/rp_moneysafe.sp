...
//Money Safe MAXSAFES
int SafeEnt[MAXSAFES + 1];
int SafeMoney[MAXSAFES + 1];
int SafeLocks[MAXSAFES + 1];
int SafeOwner[MAXSAFES + 1];
int BankOwnedSafe[MAXSAFES + 1];
int SafeRobId[MAXPLAYERS + 1];
int SafeRob[MAXSAFES + 1];
int SafeHarvest[MAXSAFES + 1];
int SafeMeth[MAXSAFES + 1];
int SafePills[MAXSAFES + 1];
int SafeCocain[MAXSAFES + 1];
int SafeRobCash[MAXPLAYERS + 1] = {0,...};
float SafeRobOrigin[MAXPLAYERS + 1][3];
char SafeName[MAXSAFES + 1][255];
...
public Action LoadMoneySafe(Handle Timer)
{
	...
	//Not Created Tables:
	SQL_TQuery(GetGlobalSQL(), T_DBLoadMoneySafe, query, 23);
}
...
public Action BeginRobberySafe(Handle Timer, any Client)
{
	...
	float Dist = GetVectorDistance(SafeRobOrigin[Client], ClientOrigin);
...
public void BeginSafeRob(int Client, int SafeCash, int X)
{
...
public void DrawMoneySafeMenu(int Client, int X)
{
...
*/
		...
		if(Result == 4)
		{
			...
			//Menu Buttons:
			menu.AddItem("1", "Take Drugs");

			menu.AddItem("2", "Store Drugs in safe");
...
//PlayerMenu Handle:
public int HandleMoneySafeDrugs(Menu menu, MenuAction HandleAction, int Client, int Parameter)
{
	...
	if(HandleAction == MenuAction_Select)
	{
		...
		if(Result == 2)
		{
			...
			if(GetHarvest(Client) > 0 || GetMeth(Client) > 0  || GetCocain(Client) > 0 || GetPills(Client) > 0)
			{
				...
				//Not Created Tables:
				SQL_TQuery(GetGlobalSQL(), SQLErrorCheckCallback, query, 28);
...
//PlayerMenu Handle:
public HandleMoneySafeWithdraw(Menu menu, MenuAction HandleAction, int Client, int Parameter)
{
...
public Action CommandCreateMoneySafe(int Client, int Args)
{
	...
	//Not Created Tables:
	SQL_TQuery(GetGlobalSQL(), SQLErrorCheckCallback, buffer, 32);
...
public Action CommandSetBankOwned(int Client, int Args)
{
	...
	for(int X = 0; X < MAXSAFES; X++)
	{
		...
		if(SafeEnt[X] == Ent)
		{
			...
			//Not Created Tables:
			SQL_TQuery(GetGlobalSQL(), SQLErrorCheckCallback, query, 41);
...
public int CreatePropMoneySafe(float Pos[3], float Ang[3], int X, int Money, int Locks, int Owner, int BankOwned, const char[] Name, int SafeH, int SafeM, int SafeP, int SafeC)
{
	...
	if(IsValidEdict(Ent))
	{
		...
		//Copy String From Buffer:
		Format(SafeName[X], sizeof(SafeName[]), "%s", Name);
	}
}

public bool IsValidMoneySafe(int Ent)
{
...
public void MoneySafeHud(int Client, int Ent)
{
